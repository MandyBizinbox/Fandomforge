...
# --- routes_main: import canonical service ---
...
# Parse before mutations for reliable function boundaries.
...
# Remove route decorator from legacy admin-create core and rename it.
...
# Make PUT a real declared method on the canonical admin product update route.
...
for deco in update_node.decorator_list:
    ...
    if '@admin_router.patch("/products/{product_id}", response_model=Product)' in text:
        replacement = '@admin_router.api_route("/products/{product_id}", methods=["PATCH", "PUT"], response_model=Product)\n'
        lines[deco.lineno - 1] = replacement
...
routes = ''.join(lines)
# Reparse after renames so insertion points are accurate.
tree = ast.parse(routes)
by_name = {node.name: node for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))}

# Canonical wrapper around existing core template normalization.
...
# Production snapshots retain Manufacturing Rule results without monkey-patching.
tree = ast.parse(routes)
by_name = {node.name: node for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))}
...
# Admin product create diagnostics become an ordinary route wrapper, not a mutated FastAPI endpoint.
tree = ast.parse(routes)
by_name = {node.name: node for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))}
...
# --- Pydantic schema ownership: Builder extension state is canonical model behavior. ---
# ProductBase -> ProductCreate inherits this; Product overrides its own config.
...
# --- server.py: remove the now-canonical monkey-patch installers. ---
for old in [
...
# Product-save was deliberately installed again at the bottom of the compatibility stack.
final_block = '''# Install Product Builder save sanitization last so no compatibility layer can\n# replace the normalizer afterwards and reintroduce server-owned Product fields.\ninstall_builder_product_save_patch(routes_main_module)\n'''
assert final_block in server
server = server.replace(final_block, '')
ast.parse(server)
server_path.write_text(server)

# Rewrite unit coverage to the canonical service and direct route/model ownership.
