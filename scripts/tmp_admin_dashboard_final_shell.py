...
block = s[start:end]
# Guard what remains before deleting it: these are obsolete inline generations, while live routes already use extracted workspaces.
...
# None of these inline components is referenced by the live route shell below.
tail = s[end:]
...
# Keep only the two tiny route leaf components, rewritten as focused modules elsewhere.
s = s[:start] + s[end:]
# Remove imports used only by the deleted legacy inline generations and leaf components moved out.
remove_lines = [
...
for line in remove_lines:
    ...
    s = s.replace(line, '')
s = s.replace('import { Navigate, Route, Routes, useNavigate, useParams } from "react-router-dom";\n', 'import { Navigate, Route, Routes } from "react-router-dom";\n') if 'useNavigate' in s.split('\n')[1] else s
# The previous exact removal removes router import entirely; restore the lean import.
if 'from "react-router-dom"' not in s:
    s = s.replace('import React, { useCallback, useEffect, useMemo, useState } from "react";\n', 'import React, { useCallback, useEffect, useMemo, useState } from "react";\nimport { Navigate, Route, Routes } from "react-router-dom";\n')
# Add focused route leaf imports.
