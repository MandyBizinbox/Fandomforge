...
def _profile_supported_colour_ids(profile: Dict[str, Any]) -> List[str]:
    return _text_list(profile.get("supported_colour_ids") or profile.get("stocked_colour_ids") or profile.get("available_colour_ids") or [])
...
def _profile_colour_mode(profile: Dict[str, Any]) -> str:
    explicit = str(profile.get("colour_selection_mode") or profile.get("color_selection_mode") or profile.get("profile_colour_mode") or "").strip().lower()
    if explicit in RESTRICTED_MODES:
        return "restricted"
    if explicit in INHERIT_MODES:
        return "inherit_method"
    has_range = bool(_profile_supported_colour_ids(profile) or _profile_available_colour_ids(profile))
...
def _validation_error(slot: Dict[str, Any], method: str, profile: Dict[str, Any], colours: List[Dict[str, Any]], code: str, message: str) -> Dict[str, Any]:
    return {"level": "error", "code": code, "message": message, "slot_id": slot.get("id"), "print_area_id": slot.get("print_area_id"), "method_key": method, "meta": {"manufacturing_profile_id": profile.get("id"), "profile_name": profile.get("display_name") or profile.get("profile_name"), "approved_colours": [colour.get("name") for colour in colours]}}


def install_profile_stocked_colours_patch(routes_production_rules_module=None, *, install_validation: bool = True) -> None:
    ...
    if not getattr(unified, "_profile_stocked_colours_patch_installed", False):
        ...
        def patched_normalize(profile: Dict[str, Any], method_key: str, *, is_default: bool = False) -> Dict[str, Any]:
            ...
            row["supported_colour_ids"] = _profile_supported_colour_ids(profile)
            row["available_colour_ids"] = _profile_available_colour_ids(profile)
            return row
    ...
    if not getattr(profiles_module, "_profile_stocked_colours_patch_installed", False):
        ...
        def patched_projection(method: Dict[str, Any], profile: Dict[str, Any]) -> Dict[str, Any]:
            ...
            mode = _profile_colour_mode(profile)
            row.update({"colour_selection_mode": mode, "color_selection_mode": mode, "supported_colour_ids": _profile_supported_colour_ids(profile), "available_colour_ids": _profile_available_colour_ids(profile), "approved_stocked_colours": colours, "stocked_colours": colours})
            return row
    ...
    if not getattr(rules_engine, "_profile_stocked_colours_patch_installed", False):
        ...
        async def patched_apply(db, product_data: dict, *, template=None, global_print_options=None, publishing: bool = False):
            data = await original_apply(db, product_data, template=template, global_print_options=global_print_options, publishing=False)
            validation = dict(data.get("production_validation") or {})
            errors = list(validation.get("errors") or [])
            method_docs = await db.production_methods.find({}, {"_id": 0}).to_list(200)
            methods = {normalize_method_key(doc.get("method_key") or doc.get("internal_id")): doc for doc in method_docs}
            for slot in rules_engine.iter_slots(data):
                method = normalize_method_key(slot.get("method_key") or slot.get("manufacturing_method_id") or slot.get("print_method"))
                ...
                if _profile_colour_mode(profile) == "restricted" and not colours:
                    errors.append(_validation_error(slot, method, profile, colours, "profile_has_no_stocked_colours", f"{profile.get('display_name') or profile.get('profile_name')} has no available stocked colours configured."))
                elif selected and not _slot_colour_is_allowed(selected, colours):
                    errors.append(_validation_error(slot, method, profile, colours, "unsupported_profile_colour", f"The selected colour is not available for {profile.get('display_name') or profile.get('profile_name')}."))
            deduped, seen = [], set()
            for error in errors:
                identity = (error.get("code"), error.get("slot_id"), str(error.get("meta", {}).get("manufacturing_profile_id") or ""))
                if identity in seen:
                    continue
                seen.add(identity)
            ...
            if publishing and validation["status"] != "valid":
                ...
                raise HTTPException(status_code=400, detail={"message": "Product cannot be published because it is not manufacturable under the active production rules.", "errors": validation.get("errors") or [], "warnings": validation.get("warnings") or [], "minimum_selling_price": data.get("minimum_selling_price"), "status": validation["status"]})
            return data
