"""Runtime integration for canonical outsourced manufacturing profiles."""
...
import unified_manufacturing_costing as unified_costing
from unified_manufacturing_costing import UNIFIED_COSTING_ENGINE_VERSION
...
def _embedded_application_methods(product_data: Dict[str, Any]) -> set[str]:
    ...
    for slot in product_data.get("artworks") or []:
        if not isinstance(slot, dict) or _number(slot.get("application_cost")) <= 0:
            continue
...
def install_outsourced_rate_runtime(routes_main_module: Any) -> None:
    ...
    original_rate_resolver = unified_costing.pricing_values_for_record

    def editable_rate_resolver(record, fallback_method=""):
        if (
            (record or {}).get("costing_engine_version") == UNIFIED_COSTING_ENGINE_VERSION
            and str((record or {}).get("id") or (record or {}).get("profile_id") or "").startswith("profile:")
        ):
            return None
        return original_rate_resolver(record, fallback_method)

    unified_costing.pricing_values_for_record = editable_rate_resolver
    ...
    def patched_raw_cost(option: Dict[str, Any], slot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if calculation_type not in {"area_fixed_rate", "area", "cm2", "sheet", "area_from_sheet"}:
    ...
    async def patched_breakdown(db, product_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for line in result.get("lines") or []:
            ...
            if method in embedded_methods and operation_type in DIRECT_APPLICATION_OPERATION_TYPES:
                continue
            lines.append(line)

        result["lines"] = lines
        result["platform_operation_cost"] = _money_half_up(sum(_number(line.get("platform_cost")) for line in lines))
        result["estimated_operation_time"] = round(sum(_number(line.get("estimated_time")) for line in lines), 2)
