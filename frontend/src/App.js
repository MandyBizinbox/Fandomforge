...
const AdminAccountReview = lazy(() => import("./pages/AdminAccountReview"));
...
function RouteLoading() {
  return <div className="min-h-[55vh] page-shell flex items-center justify-center px-6"><div className="overline">Loading FandomForge…</div></div>;
}
...
function Protected({ roles, children }) {
  ...
  if (roles && !roles.includes(user.role)) {
    ...
    return <div className="min-h-screen page-shell flex items-center justify-center px-6"><div className="card max-w-md"><p className="overline mb-2">Access issue</p><h1 className="font-display text-3xl uppercase mb-3">Account role mismatch</h1><p className="text-[var(--ff-muted-text)] text-sm mb-4">Your account role is not allowed for this page.</p><p className="text-[var(--ff-muted-text)] text-xs">Current role: {user.role || "unknown"}</p></div></div>;
  }
  return children;
}
