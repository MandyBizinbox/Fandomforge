...
function buildAdminLinks(basePath = "/admin") {
  return [
...
function filterAdminLinks({ modules = {}, user = null, basePath = "/admin", mode = "admin" } = {}) {
...
function normaliseEditableViewKey(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/[^a-z0-9_]+/g, "_")
    .replace(/_+/g, "_")
...
function viewKeyExists(screens = [], key, ignoreId = "") {
  const normalised = makeViewKey(key);
  return screens.some((screen) => screen.id !== ignoreId && makeViewKey(screenViewKey(screen)) === normalised);
}

function screenPrintDefaults(screen = {}) {
  const view = screenViewKey(screen);
  const isFullWrap = ["full_wrap", "mug_wrap"].includes(view);
  return {
    width_mm: Number(screen.default_width_mm || screen.width_mm || (isFullWrap ? 220 : 210)),
    height_mm: Number(screen.default_height_mm || screen.height_mm || (isFullWrap ? 90 : 297)),
    dpi: Number(screen.default_dpi || screen.dpi || 300),
    print_size: screen.default_print_size || (isFullWrap ? "Full Wrap" : "A4"),
    standard_print_size_key: screen.default_standard_print_size_key || (isFullWrap ? "full_wrap" : "a4_portrait"),
  };
}
...
function defaultPrintAreaForScreen(screen = {}, areaCount = 0) {
  const view = screenViewKey(screen);
  const isFullWrap = ["full_wrap", "mug_wrap"].includes(view);
  const defaults = screenPrintDefaults(screen);
  return {
    screen_id: screen.id,
    screen_view: view,
    view_key: view,
    area_key: isFullWrap ? "full_wrap" : view,
    name: isFullWrap ? `Full Wrap Area ${areaCount + 1}` : `${screen.name || "Print"} Area ${areaCount + 1}`,
    print_size: defaults.print_size,
    standard_print_size_key: defaults.standard_print_size_key,
...
function Overview() {
  ...
  return (
    <div data-testid="admin-overview">
      ...
      <button
        type="button"
        onClick={() => window.location.assign("/admin/artwork-review")}
        className="card card-interactive mb-6 w-full text-left flex flex-col md:flex-row md:items-center md:justify-between gap-3"
      >
        <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 border-t border-l border-[var(--ff-card-border)]">
        ...
        {card("Active Creators", stats.bands_active, "text-[#34C759]")}
        {card("Printers", stats.printers)}
        {card("Products", stats.products)}
        {card("Total Orders", stats.orders_total)}
        {card("Paid Orders", stats.orders_paid, "text-[#34C759]")}
...
function ProductTemplatesAdmin() {
  const [rows, setRows] = useState([]);
  const navigate = useNavigate();
  const load = () => http.get("/admin/product-templates").then((r) => setRows(r.data));
  useEffect(() => { load(); }, []);
  const remove = async (template) => {
    if (!window.confirm(`Archive/delete "${template.name}"?`)) return;
    try { await http.delete(`/admin/product-templates/${template.id}`); toast.success("Template removed or archived"); load(); }
    catch (e) { toast.error(e.response?.data?.detail || "Failed"); }
  };
  return (
    <div data-testid="admin-template-list">
      <div className="flex items-center justify-between mb-8">
        <div><div className="overline mb-2">Blank Catalog</div><h1 className="font-display text-5xl uppercase">Product Templates</h1></div>
        <button onClick={() => navigate("/admin/product-templates/new")} className="btn-primary"><Plus size={14} /> New Template</button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {rows.map((t) => (
          <div key={t.id} className="border border-[var(--ff-card-border)] bg-[var(--ff-card-bg)] hover:border-[var(--ff-primary)]/70 transition" data-testid={`template-card-${t.id}`}>
            <div className="aspect-square bg-[var(--ff-card-bg)] border-b border-[var(--ff-card-border)] flex items-center justify-center overflow-hidden">
              {t.product_image_url || t.mockup_url ? <img src={assetUrl(t.product_image_url || t.mockup_url)} alt={t.name} className="w-full h-full object-contain" /> : <ImageIcon className="text-[var(--ff-muted-text)]" size={54} />}
            </div>
            <div className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div><h3 className="font-display text-2xl uppercase leading-none">{t.name}</h3><p className="text-xs text-[var(--ff-muted-text)] mt-2">{t.brand || "No brand"} · {t.category}</p></div>
                <StatusBadge status={t.status || "draft"} />
              </div>
              <div className="grid grid-cols-3 gap-2 my-4 text-center text-xs uppercase tracking-widest text-[var(--ff-muted-text)]">
                <div className="border border-[var(--ff-card-border)] p-2"><b className="block text-[var(--ff-card-text)] text-lg">{(t.mockup_screens || []).length}</b> Views</div>
                <div className="border border-[var(--ff-card-border)] p-2"><b className="block text-[var(--ff-card-text)] text-lg">{(t.print_areas || []).length}</b> Areas</div>
                <div className="border border-[var(--ff-card-border)] p-2"><b className="block text-[var(--ff-card-text)] text-lg">{(t.variations || []).length}</b> Vars</div>
              </div>
              <div className="flex gap-2"><button onClick={() => navigate(`/admin/product-templates/${t.id}`)} className="btn-primary flex-1">Open Studio</button><button onClick={() => remove(t)} className="px-3 border border-[var(--ff-card-border)] hover:border-[var(--ff-primary)] text-[var(--ff-muted-text)]"><Trash2 size={16} /></button></div>
            </div>
          </div>
        ))}
      </div>
      {rows.length === 0 && <div className="card text-center text-[var(--ff-muted-text)] overline">No templates yet</div>}
    </div>
  );
}

function PrintAreaCanvas({ screen, areas, selectedAreaId, onSelectArea, onCreateArea, onPatchArea }) {
  ...
  const startDraw = (e) => {
    if (!screen?.image_url || mode !== "draw") return;
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--ff-card-bg)]">
      <div className="h-14 border-b border-[var(--ff-card-border)] flex items-center justify-between px-4">
        <div><div className="overline">Canvas</div><div className="text-xs text-[var(--ff-muted-text)]">{screen?.name || "No view selected"}</div></div>
        <div className="flex gap-2"><button type="button" onClick={() => setMode("select")} className={`px-3 py-2 text-xs uppercase tracking-widest border ${mode === "select" ? "border-[var(--ff-primary)] text-[var(--ff-card-text)]" : "border-[var(--ff-card-border)] text-[var(--ff-muted-text)]"}`}>Select</button><button type="button" onClick={() => setMode("draw")} className={`px-3 py-2 text-xs uppercase tracking-widest border ${mode === "draw" ? "border-[var(--ff-primary)] text-[var(--ff-card-text)]" : "border-[var(--ff-card-border)] text-[var(--ff-muted-text)]"}`}>Draw Area</button></div>
      </div>
      <div className="flex-1 overflow-auto p-8 flex items-center justify-center">
        {!screen?.image_url ? <div className="text-center text-[var(--ff-muted-text)]"><ImageIcon size={64} className="mx-auto mb-4" /><div className="overline">Upload a product view to start drawing</div></div> : (
          <div ref={boxRef} className="relative max-w-full max-h-full select-none touch-none" onMouseDown={startDraw} onMouseMove={movePointer} onMouseUp={endPointer} onMouseLeave={endPointer}>
            ...
            {draft && <div className="absolute border-2 border-dashed border-[#FFCC00] bg-[var(--ff-surface-bg)]" style={{ left: `${draft.x}%`, top: `${draft.y}%`, width: `${draft.width}%`, height: `${draft.height}%` }} />}
          </div>
        )}
      </div>
    </div>
...
function ProductTemplateStudio() {
  ...
  const activeScreen = useMemo(() => (template.mockup_screens || []).find((s) => s.id === activeScreenId) || (template.mockup_screens || [])[0] || null, [template.mockup_screens, activeScreenId]);
  const screenAreas = useMemo(() => (template.print_areas || []).filter((a) => a.screen_id === activeScreen?.id), [template.print_areas, activeScreen]);
  const selectedArea = useMemo(() => (template.print_areas || []).find((a) => a.id === selectedAreaId) || null, [template.print_areas, selectedAreaId]);

  useEffect(() => {
    http.get("/categories").then((r) => setCategories(r.data)).catch(() => {});
    http.get("/attributes").then((r) => setAttributes(r.data)).catch(() => {});
    http.get("/print-options").then((r) => setPrintOptions(r.data)).catch(() => {});
    if (!isNew) {
      http.get(`/admin/product-templates/${id}`).then((r) => {
        const next = { ...blankTemplate, ...r.data };
        setTemplate(next);
        setActiveScreenId((next.mockup_screens || [])[0]?.id || null);
      }).catch((e) => toast.error(e.response?.data?.detail || "Could not load template"));
    }
  }, [id, isNew]);

  const patch = (updates) => setTemplate((prev) => ({ ...prev, ...updates }));
  const uploadImage = async (file, subdir = "product-templates") => {
    if (!file) return null;
    setUploading(true);
    const data = new FormData();
    data.append("file", file);
    data.append("subdir", subdir);
    try { const r = await http.post("/files/image", data); return r.data.url; }
  ...
  const addScreen = async (file, name = "Front", view = "front") => {
    ...
    if (!url) return;
    const viewKey = makeViewKey(view || name);
    const screen = { id: newId("screen"), name, view: viewKey, label: name, view_key: viewKey, screen_view: viewKey, image_url: url, sort_order: template.mockup_screens.length, is_primary: template.mockup_screens.length === 0 };
  ...
  const addCustomScreen = (screenDraft) => {
    const name = String(screenDraft.name || "").trim();
    const viewKey = makeViewKey(screenDraft.view_key || name);
    if (!name) { toast.error("Enter a view label."); return; }
    if (!viewKey) { toast.error("Enter a valid view key."); return; }
    if (viewKeyExists(template.mockup_screens, viewKey)) { toast.error("View key must be unique for this template."); return; }
    ...
    const screen = {
      ...
      notes: screenDraft.description || "",
      default_width_mm: screenDraft.default_width_mm ? Number(screenDraft.default_width_mm) : undefined,
      default_height_mm: screenDraft.default_height_mm ? Number(screenDraft.default_height_mm) : undefined,
      default_dpi: screenDraft.default_dpi ? Number(screenDraft.default_dpi) : 300,
      image_url: "",
      sort_order: template.mockup_screens.length,
      is_primary: Boolean(screenDraft.is_primary) || template.mockup_screens.length === 0,
    };

    const screens = screen.is_primary ? template.mockup_screens.map((item) => ({ ...item, is_primary: false })) : template.mockup_screens;
    patch({ mockup_screens: [...screens, screen] });
    setActiveScreenId(screen.id);
    toast.success("Custom base view added. Upload an image from View Settings.");
  };

  const createArea = (area) => {
    const next = { id: newId("area"), ...area };
  ...
  const generateVariations = () => {
    const selected = attributes.filter((a) => template.attribute_ids.includes(a.id) && a.used_for_variation);
    if (!selected.length) { toast.error("Select at least one variation attribute"); return; }
    const combos = selected.reduce((acc, attr) => acc.flatMap((row) => (attr.values || []).map((v) => ({ ...row, [attr.name]: v }))), [{}]);
    patch({ variations: combos.map((attrs) => ({ id: newId("var"), attributes: attrs, sku: "", supplier_sku: "", cost: Number(template.base_blank_cost || 0), base_blank_cost: Number(template.base_blank_cost || 0), status: "active" })) });
    toast.success(`Generated ${combos.length} variations`);
  };
  ...
  const save = async () => {
    ...
    const viewKeys = (template.mockup_screens || []).map((screen) => makeViewKey(screenViewKey(screen)));
    if (viewKeys.some((key) => !key)) { toast.error("Every base view needs a valid view key."); return; }
    if (new Set(viewKeys).size !== viewKeys.length) { toast.error("Base view keys must be unique inside this template."); return; }
    setSaving(true);
    const payload = {
      ...template,
      base_price: Number(template.base_blank_cost || template.base_price || 0),
      base_blank_cost: Number(template.base_blank_cost || template.base_price || 0),
      print_options: printOptions
        .filter((o) => template.print_option_ids.includes(o.id))
        .map((o) => snapshotPrintRule(o)),
    };
    try {
      const r = isNew ? await http.post("/admin/product-templates", payload) : await http.patch(`/admin/product-templates/${id}`, payload);
      toast.success("Template saved");
      if (isNew) navigate(`/admin/product-templates/${r.data.id}`);
      else setTemplate({ ...blankTemplate, ...r.data });
    } catch (e) { toast.error(e.response?.data?.detail || "Save failed"); }
    finally { setSaving(false); }
  };

  return (
    <div className="fixed inset-0 bg-[var(--ff-page-bg)] text-[var(--ff-card-text)] z-50 flex flex-col" data-testid="template-studio">
      <header className="h-16 border-b border-[var(--ff-card-border)] flex items-center justify-between px-4 bg-[var(--ff-page-bg)]">
        <div className="flex items-center gap-4"><button onClick={() => navigate("/admin/product-templates")} className="border border-[var(--ff-card-border)] p-2 hover:border-white"><ChevronLeft size={18} /></button><div><div className="overline">Product Template Studio</div><h1 className="font-display text-2xl uppercase leading-none">{template.name || "New Blank Product"}</h1></div></div>
        <div className="flex items-center gap-2"><StatusBadge status={template.status || "draft"} /><button onClick={save} disabled={saving} className="btn-primary"><Save size={14} /> {saving ? "Saving…" : "Save Template"}</button></div>
      </header>
      <div className="flex-1 grid grid-cols-[310px_1fr_360px] min-h-0">
        <aside className="border-r border-[var(--ff-card-border)] bg-[var(--ff-card-bg)] overflow-y-auto">
          <div className="p-4 border-b border-[var(--ff-card-border)]">
            <div className="grid grid-cols-2 gap-2">
              {["setup", "views", "areas", "variations", "methods"].map((p) => <button key={p} onClick={() => setActivePanel(p)} className={`px-3 py-2 text-xs uppercase tracking-widest border ${activePanel === p ? "border-[var(--ff-primary)] text-[var(--ff-card-text)]" : "border-[var(--ff-card-border)] text-[var(--ff-muted-text)]"}`}>{p}</button>)}
            </div>
          </div>
          {activePanel === "setup" && <div className="p-4 space-y-3">
            <div><label className="label">Template Name</label><input className="input-base" value={template.name} onChange={(e) => patch({ name: e.target.value })} /></div>
            <div><label className="label">Category</label><select className="input-base" value={template.category_id || ""} onChange={(e) => { const cat = categories.find((c) => c.id === e.target.value); patch({ category_id: cat?.id || "", category: cat?.slug || "" }); }}><option value="">— pick —</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
            <div><label className="label">Brand</label><input className="input-base" value={template.brand || ""} onChange={(e) => patch({ brand: e.target.value })} /></div>
            <div><label className="label">Blank SKU</label><input className="input-base" value={template.blank_sku || ""} onChange={(e) => patch({ blank_sku: e.target.value })} /></div>
            <div><label className="label">Base Blank Cost</label><input type="number" step="0.01" className="input-base" value={template.base_blank_cost || 0} onChange={(e) => patch({ base_blank_cost: e.target.value })} /></div>
            <div><label className="label">Supplier Name</label><input className="input-base" value={template.supplier_name || ""} onChange={(e) => patch({ supplier_name: e.target.value })} /></div>
            <div><label className="label">Supplier URL</label><input className="input-base" value={template.supplier_url || ""} onChange={(e) => patch({ supplier_url: e.target.value })} /></div>
            <div><label className="label">Description</label><textarea rows={3} className="input-base" value={template.description || ""} onChange={(e) => patch({ description: e.target.value })} /></div>
          ...
          {activePanel === "views" && <div className="p-4 space-y-4">
            <div className="card"><div className="overline mb-3">Add Base View</div><ViewUpload template={template} onAdd={addScreen} onAddCustom={addCustomScreen} uploading={uploading} /></div>
            <div className="space-y-2">{template.mockup_screens.map((s) => <button key={s.id} onClick={() => setActiveScreenId(s.id)} className={`w-full border ${activeScreen?.id === s.id ? "border-[var(--ff-primary)]" : "border-[var(--ff-card-border)]"} p-2 flex gap-3 text-left`}>{s.image_url ? <img src={assetUrl(s.image_url)} alt="" className="w-16 h-16 object-contain bg-[var(--ff-card-bg)]" /> : <div className="w-16 h-16 bg-[var(--ff-card-bg)] border border-[var(--ff-card-border)] flex items-center justify-center text-[var(--ff-muted-text)]"><ImageIcon size={18} /></div>}<span><b className="block uppercase">{s.name}</b><span className="text-xs text-[var(--ff-muted-text)]">{viewLabel(screenViewKey(s))}</span></span></button>)}</div>
          </div>}
          ...
          {activePanel === "methods" && <div className="p-4 space-y-3"><div className="overline">Allowed Pricing Rules</div>{printOptions.map((o) => <label key={o.id} className="flex gap-2 text-sm border border-[var(--ff-card-border)] p-3"><input type="checkbox" checked={template.print_option_ids.includes(o.id)} onChange={(e) => patch({ print_option_ids: e.target.checked ? [...template.print_option_ids, o.id] : template.print_option_ids.filter((id) => id !== o.id) })} /><span><b>{printRuleLabel(o)}</b><span className="block text-xs text-[var(--ff-muted-text)]">{printRulePricingSummary(o)}</span><span className="block text-xs text-[var(--ff-muted-text)]">Output: {o.print_size || "dynamic"} · Areas: {(o.print_positions || []).join(", ") || "none"}</span></span></label>)}</div>}
        </aside>
        <main className="min-w-0 min-h-0"><PrintAreaCanvas screen={activeScreen} areas={screenAreas} selectedAreaId={selectedAreaId} onSelectArea={setSelectedAreaId} onCreateArea={createArea} onPatchArea={patchArea} /></main>
        <aside className="border-l border-[var(--ff-card-border)] bg-[var(--ff-card-bg)] overflow-y-auto"><Inspector template={template} patch={patch} screen={activeScreen} selectedArea={selectedArea} patchArea={patchArea} deleteArea={deleteArea} printOptions={printOptions} uploadImage={uploadImage} uploading={uploading} /></aside>
      </div>
    </div>
  );
}

function ViewUpload({ template, onAdd, onAddCustom, uploading }) {
  ...
  const [custom, setCustom] = useState({ name: "", view_key: "", description: "", default_width_mm: "", default_height_mm: "", default_dpi: "300", is_primary: false });
  ...
  const setCustomName = (value) => {
    setCustom((current) => ({ ...current, name: value, view_key: current.view_key_touched ? current.view_key : makeViewKey(value) }));
  };

  const setCustomKey = (value) => {
    setCustom((current) => ({ ...current, view_key: normaliseEditableViewKey(value), view_key_touched: true }));
  };

  const customKeyError = custom.view_key && viewKeyExists(template.mockup_screens, custom.view_key) ? "View key already exists on this template." : "";

  const saveCustom = () => {
    if (!custom.name.trim()) { toast.error("Enter a view label."); return; }
    if (!makeViewKey(custom.view_key || custom.name)) { toast.error("Enter a valid view key."); return; }
    if (customKeyError) { toast.error(customKeyError); return; }
    onAddCustom?.(custom);
    setShowCustom(false);
    setCustom({ name: "", view_key: "", description: "", default_width_mm: "", default_height_mm: "", default_dpi: "300", is_primary: false });
  };
    ...
    <div className="space-y-3">
      ...
      {showCustom && (
        <div className="border border-[var(--ff-card-border)] bg-black/20 p-3 space-y-3">
          <div><label className="label">View label</label><input className="input-base" value={custom.name} onChange={(e) => setCustomName(e.target.value)} placeholder="Lid Top" /></div>
          <div><label className="label">View key / type</label><input className="input-base" value={custom.view_key} onChange={(e) => setCustomKey(e.target.value)} placeholder="lid_top" />{customKeyError && <div className="text-xs text-[var(--ff-primary)] mt-1">{customKeyError}</div>}</div>
          <div><label className="label">Description / notes</label><textarea className="input-base" rows={2} value={custom.description} onChange={(e) => setCustom((current) => ({ ...current, description: e.target.value }))} /></div>
          <div className="grid grid-cols-3 gap-2">
            <div><label className="label">Width mm</label><input type="number" className="input-base" value={custom.default_width_mm} onChange={(e) => setCustom((current) => ({ ...current, default_width_mm: e.target.value }))} /></div>
            <div><label className="label">Height mm</label><input type="number" className="input-base" value={custom.default_height_mm} onChange={(e) => setCustom((current) => ({ ...current, default_height_mm: e.target.value }))} /></div>
            <div><label className="label">DPI</label><input type="number" className="input-base" value={custom.default_dpi} onChange={(e) => setCustom((current) => ({ ...current, default_dpi: e.target.value }))} /></div>
          </div>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={custom.is_primary} onChange={(e) => setCustom((current) => ({ ...current, is_primary: e.target.checked }))} /> Make primary view</label>
          <button type="button" className="btn-primary w-full justify-center" onClick={saveCustom}>Save Custom View</button>
        </div>
      )}
    </div>
  );
...
function Inspector({ template, patch, screen, selectedArea, patchArea, deleteArea, printOptions, uploadImage, uploading }) {
  ...
  if (screen) {
    ...
    const duplicateKey = viewKeyExists(template.mockup_screens, currentKey, screen.id);
    const patchScreen = (updates) => patch({ mockup_screens: template.mockup_screens.map((s) => s.id === screen.id ? { ...s, ...updates } : s) });

    return (
      <div className="p-4 space-y-4">
        <div><div className="overline">Inspector</div><h2 className="font-display text-3xl uppercase">View Settings</h2></div>
        <div className="aspect-square bg-[var(--ff-card-bg)] border border-[var(--ff-card-border)] flex items-center justify-center overflow-hidden">
          {screen.image_url ? <img src={assetUrl(screen.image_url)} alt="" className="w-full h-full object-contain" /> : <div className="text-center text-[var(--ff-muted-text)]"><ImageIcon size={42} className="mx-auto mb-3" /><div className="overline">No image uploaded</div></div>}
        </div>
        <div><label className="label">View Name</label><input className="input-base" value={screen.name} onChange={(e) => patchScreen({ name: e.target.value, label: e.target.value })} /></div>
        <div><label className="label">View Type</label><input className="input-base" value={currentKey} onChange={(e) => { const key = normaliseEditableViewKey(e.target.value); patch({ mockup_screens: template.mockup_screens.map((s) => s.id === screen.id ? { ...s, view: key, view_key: key, screen_view: key } : s), print_areas: template.print_areas.map((area) => area.screen_id === screen.id ? { ...area, screen_view: key, view_key: key, area_key: area.area_key === currentKey ? key : area.area_key } : area) }); }} />{duplicateKey && <div className="text-xs text-[var(--ff-primary)] mt-1">View key must be unique inside this template.</div>}</div>
        <div><label className="label">Description / notes</label><textarea className="input-base" rows={2} value={screen.description || screen.notes || ""} onChange={(e) => patchScreen({ description: e.target.value, notes: e.target.value })} /></div>
        <div className="grid grid-cols-3 gap-2">
          <div><label className="label">Default width mm</label><input type="number" className="input-base" value={screen.default_width_mm || ""} onChange={(e) => patchScreen({ default_width_mm: e.target.value ? Number(e.target.value) : undefined })} /></div>
          <div><label className="label">Default height mm</label><input type="number" className="input-base" value={screen.default_height_mm || ""} onChange={(e) => patchScreen({ default_height_mm: e.target.value ? Number(e.target.value) : undefined })} /></div>
          <div><label className="label">Default DPI</label><input type="number" className="input-base" value={screen.default_dpi || ""} onChange={(e) => patchScreen({ default_dpi: e.target.value ? Number(e.target.value) : undefined })} /></div>
        </div>
        <label className="flex gap-2 text-sm"><input type="checkbox" checked={!!screen.is_primary} onChange={() => patch({ mockup_screens: template.mockup_screens.map((s) => ({ ...s, is_primary: s.id === screen.id })), mockup_url: screen.image_url, product_image_url: screen.image_url })} /> Primary product image</label>
        <label className="btn-primary w-full cursor-pointer justify-center"><ImageIcon size={14} /> {uploading ? "Uploading…" : screen.image_url ? "Replace Image" : "Upload Image"}<input type="file" accept="image/*" className="hidden" onChange={async (e) => { const url = await uploadImage(e.target.files?.[0], "product-template-views"); if (url) patchScreen({ image_url: url }); }} /></label>
      </div>
    );
  }
...
function VariationMatrix({ rows, onChange }) {
  if (!rows.length) return <div className="text-xs text-[var(--ff-muted-text)] overline">No variations generated</div>;
  return <div className="space-y-2 max-h-[420px] overflow-y-auto">{rows.map((row, idx) => <div key={row.id || idx} className="border border-[var(--ff-card-border)] p-3 space-y-2"><div className="text-xs uppercase tracking-widest text-[var(--ff-muted-text)]">{Object.entries(row.attributes || {}).map(([k, v]) => `${k}: ${v}`).join(" / ")}</div><div className="grid grid-cols-2 gap-2"><input className="input-base text-xs" placeholder="SKU" value={row.sku || ""} onChange={(e) => onChange(rows.map((r, i) => i === idx ? { ...r, sku: e.target.value } : r))} /><input type="number" className="input-base text-xs" placeholder="Cost" value={row.cost || 0} onChange={(e) => onChange(rows.map((r, i) => i === idx ? { ...r, cost: Number(e.target.value), base_blank_cost: Number(e.target.value) } : r))} /></div></div>)}</div>;
}
...
const PRINT_SIZE_PRESETS = [
  {
    key: "sleeve_large_20x30_cm",
    label: "Sleeve Large",
    print_size: "Sleeve Large 20×30cm",
    width_mm: 200,
    height_mm: 300,
    dpi: 300,
    fit_mode: "contain",
  ...
  {
    key: "sleeve_long_10x30_cm",
    label: "Sleeve Long",
    print_size: "Sleeve Long 10×30cm",
    width_mm: 100,
    height_mm: 300,
    dpi: 300,
    fit_mode: "contain",
  ...
  {
    key: "small_square_5x5_cm",
    label: "Small Square",
    print_size: "Small Square 5×5cm",
    width_mm: 50,
    height_mm: 50,
    dpi: 300,
    fit_mode: "contain",
    positions: ["front", "back", "sleeve", "pocket"],
  },
  {
    key: "neck_label_wide_9x2_cm",
    label: "Neck Label Wide",
    print_size: "Neck Label Wide 9×2cm",
    width_mm: 90,
    height_mm: 20,
    dpi: 300,
    fit_mode: "contain",
    positions: ["neck_label"],
  },
  {
    key: "a4_portrait",
    label: "A4 Portrait",
    print_size: "A4 Portrait 21×29.7cm",
    width_mm: 210,
    height_mm: 297,
    dpi: 300,
    fit_mode: "contain",
  ...
  },
  {
    key: "a3_portrait",
    label: "A3 Portrait",
    print_size: "A3 Portrait 29.7×42cm",
    width_mm: 297,
    height_mm: 420,
    dpi: 300,
    fit_mode: "contain",
...
function printRuleLabel(option) {
  return option?.rule_name || option?.print_method || option?.method || "Pricing rule";
}

function printRulePricingSummary(option) {
  const type = option?.calculation_type || "fixed";
  if (type === "fixed") return `Fixed · ${money(option?.print_cost_max || 0)}`;
  if (type === "area_from_sheet") return `Area from sheet · R ${Number(option?.cost_per_cm2 || 0).toFixed(4)}/cm² · sheet ${option?.sheet_width_mm || 0}×${option?.sheet_height_mm || 0}mm`;
  if (type === "area_fixed_rate") return `Area fixed rate · R ${Number(option?.cost_per_cm2 || 0).toFixed(4)}/cm² · min ${money(option?.minimum_print_cost || 0)}`;
  if (type === "sheet") return `Full sheet · ${money(option?.sheet_cost || 0)}`;
  return type;
}

function snapshotPrintRule(option) {
  return {
    id: option.id,
    rule_name: option.rule_name || option.print_method || "",
    print_method: option.print_method || option.rule_name || "",
    method: option.print_method || option.rule_name || "",
    method_key: option.method_key || "",
    print_size: option.print_size || "",
    standard_print_size_key: option.standard_print_size_key || "",
    width_mm: Number(option.width_mm || 0),
    height_mm: Number(option.height_mm || 0),
    dpi: Number(option.dpi || 300),
    fit_mode: option.fit_mode || "contain",
    print_cost_max: Number(option.print_cost_max || 0),
    print_positions: option.print_positions || [],
    ...
    production_notes: option.production_notes || "",
...
function PrintOptionsAdmin() {
  ...
  const displayName = (row) => row.rule_name || row.print_method || "Untitled pricing rule";

  const resolvePrintSize = (draft) => {
  ...
  const resolveStandardKey = (draft) => {
  ...
  const makePayload = () => {
    ...
    const methodKey = (form.method_key || "").trim();
  ...
  const togglePlacement = (key) => {
    const current = splitCsv(form.print_positions);
    const next = current.includes(key)
      ? current.filter((item) => item !== key)
      : [...current, key];

    setForm({ ...form, print_positions: next.join(", ") });
  };
  ...
  const save = async (e) => {
    ...
    try {
      if (editingId) {
        await http.patch(`/print-options/${editingId}`, payload);
        toast.success("Pricing rule updated");
      } else {
        await http.post("/print-options", payload);
      ...
      toast.error(err.response?.data?.detail || "Failed to save pricing rule");
    }
  };

  const remove = async (row) => {
    if (!window.confirm(`Delete/archive ${displayName(row)}?`)) return;
  ...
  const renderForm = () => (
    <form onSubmit={save} className="card space-y-6 max-w-5xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="font-display text-4xl uppercase">
            {editingId ? form.rule_name || "Pricing Rule" : "New Pricing Rule"}
          </h2>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <label>
          <span className="label">Rule Name / Print Method Title</span>
          <input
            className="input-base"
            required
            placeholder="Example: DTF - Area From Sheet"
        ...
        <label>
          <span className="label">Method Key / Category</span>
          <input
            className="input-base"
            placeholder="Example: dtf, embroidery, dtf_area_sheet"
        ...
        <label className="lg:col-span-2">
          ...
          <select className="input-base" value={form.calculation_type || "fixed"} onChange={(e) => setForm({ ...form, calculation_type: e.target.value })}>
            <option value="fixed">Fixed</option>
            <option value="area_from_sheet">Area from sheet</option>
            <option value="area_fixed_rate">Area fixed rate per cm²</option>
      ...
      {form.calculation_type === "fixed" && (
        <div className="border border-white/15 rounded-xl p-5 space-y-4">
          <div>
            <div className="overline mb-1">Fixed Output</div>
            <p className="text-xs text-zinc-400">Use this when the rule has a fixed output size or fixed print cost.</p>
          </div>

          <label>
            <span className="label">Size preset</span>
            <select className="input-base" value={form.standard_print_size_key} onChange={(e) => applySizePreset(e.target.value)}>
              <option value="">No size preset / custom</option>
              {PRINT_SIZE_PRESETS.map((preset) => (
                <option key={preset.key} value={preset.key}>{preset.label} · {preset.print_size}</option>
              ))}
            </select>
          </label>

          <label>
            <span className="label">Print size label</span>
            <input className="input-base" value={form.print_size} onChange={(e) => setForm({ ...form, print_size: e.target.value })} />
          </label>

          <div className="grid md:grid-cols-2 gap-3">
            <label>
              <span className="label">Width mm</span>
              <input className="input-base" type="number" value={form.width_mm} onChange={(e) => setForm({ ...form, width_mm: e.target.value })} />
            </label>
            <label>
              <span className="label">Height mm</span>
              <input className="input-base" type="number" value={form.height_mm} onChange={(e) => setForm({ ...form, height_mm: e.target.value })} />
            </label>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <label>
              ...
              <input className="input-base" type="number" value={form.dpi} onChange={(e) => setForm({ ...form, dpi: e.target.value })} />
            </label>
            <label>
              <span className="label">Fit mode</span>
              <select className="input-base" value={form.fit_mode} onChange={(e) => setForm({ ...form, fit_mode: e.target.value })}>
          ...
          <label>
            <span className="label">Fixed print cost</span>
            <input type="number" step="0.01" className="input-base" value={form.print_cost_max} onChange={(e) => setForm({ ...form, print_cost_max: e.target.value })} />
          </label>
        </div>
      )}
      ...
      {form.calculation_type !== "fixed" && (
        <div className="border border-white/15 rounded-xl p-5 space-y-4">
          <div>
            <div className="overline mb-1">Pricing Calculation</div>
            <p className="text-xs text-zinc-400">Only fields required for the selected calculation type are shown.</p>
          </div>

          {["area_from_sheet", "sheet"].includes(form.calculation_type) && (
            <div className="grid md:grid-cols-3 gap-3">
              <label>
                <span className="label">Sheet W mm</span>
                <input type="number" step="0.01" className="input-base" value={form.sheet_width_mm} onChange={(e) => recalcCostPerCm2({ sheet_width_mm: e.target.value })} />
              </label>
              <label>
                <span className="label">Sheet H mm</span>
                <input type="number" step="0.01" className="input-base" value={form.sheet_height_mm} onChange={(e) => recalcCostPerCm2({ sheet_height_mm: e.target.value })} />
              </label>
              <label>
                <span className="label">Sheet cost</span>
                <input type="number" step="0.01" className="input-base" value={form.sheet_cost} onChange={(e) => recalcCostPerCm2({ sheet_cost: e.target.value })} />
              </label>
            </div>
          )}

          {["area_from_sheet", "area_fixed_rate"].includes(form.calculation_type) && (
            <div className="grid md:grid-cols-2 gap-3">
              <label>
                <span className="label">Cost per cm²</span>
                <input type="number" step="0.000001" className="input-base" value={form.cost_per_cm2} onChange={(e) => setForm({ ...form, cost_per_cm2: e.target.value })} />
              </label>
              <label>
                <span className="label">Minimum print cost</span>
                <input type="number" step="0.01" className="input-base" value={form.minimum_print_cost} onChange={(e) => setForm({ ...form, minimum_print_cost: e.target.value })} />
              </label>
            </div>
          )}
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <label>
              ...
              <input type="number" step="0.01" className="input-base" value={form.waste_percentage} onChange={(e) => setForm({ ...form, waste_percentage: e.target.value })} />
            </label>
            <label>
              <span className="label">Markup %</span>
              <input type="number" step="0.01" className="input-base" value={form.markup_percentage} onChange={(e) => setForm({ ...form, markup_percentage: e.target.value })} />
            </label>
          </div>

          <div className="text-xs text-zinc-400">
            Estimated cm² rate: <b className="text-white">R {Number(form.cost_per_cm2 || 0).toFixed(4)}</b>
            {Number(form.sheet_width_mm || 0) > 0 && Number(form.sheet_height_mm || 0) > 0 && (
              <span> · Sheet area: {(((Number(form.sheet_width_mm || 0) / 10) * (Number(form.sheet_height_mm || 0) / 10))).toFixed(0)}cm²</span>
            )}
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {PRINT_PLACEMENT_TAGS.map((tag) => {
            const active = splitCsv(form.print_positions).includes(tag.key);
            return (
              <button
                type="button"
                key={tag.key}
                onClick={() => togglePlacement(tag.key)}
                className={`px-3 py-2 border text-xs uppercase tracking-widest font-bold ${
                  active ? "border-[#FF3B30] bg-[#FF3B30] text-white" : "border-white/15 text-zinc-400 hover:text-white"
                }`}
              >
                {tag.label}
              </button>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <label>
          <span className="label">Pricing notes</span>
          <textarea className="input-base" rows={3} value={form.pricing_notes} onChange={(e) => setForm({ ...form, pricing_notes: e.target.value })} />
        </label>
        <label>
          <span className="label">Production notes</span>
          <textarea className="input-base" rows={3} value={form.production_notes} onChange={(e) => setForm({ ...form, production_notes: e.target.value })} />
        </label>
      </div>
      ...
      <div className="border border-white/15 bg-black rounded-xl p-4 text-xs text-zinc-400">
        <div className="font-bold text-white mb-1">Output preview</div>
      ...
      <div className="flex flex-wrap gap-3">
        <button className="btn-primary"><Save size={14} /> {editingId ? "Save Changes" : "Create Pricing Rule"}</button>
  ...
  return (
    <div data-testid="admin-print-options-page" className="space-y-8">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          <div className="overline mb-2">Product-side print taxonomy</div>
          <h1 className="font-display text-5xl uppercase">Print Method Pricing</h1>
          <p className="text-zinc-400 mt-3 max-w-4xl">
            Define how each print method or pricing rule is calculated. Product templates decide which rules are allowed per print area.
          </p>
        </div>

        {mode === "list" && (
          <button type="button" className="btn-primary" onClick={startCreate}>+ New Pricing Rule</button>
        )}
      </div>
      ...
      {mode !== "list" ? renderForm() : (
        <div className="border border-white/15 overflow-x-auto">
          <table className="table-brutal min-w-[1120px]">
            ...
            <tbody>
              {loadingRows && (
                <tr><td colSpan={8} className="p-10 text-center text-zinc-500 overline">Loading pricing rules…</td></tr>
              )}

              {!loadingRows && rows.map((r) => (
                <tr key={r.id}>
                  <td>
                    <div className="font-bold">{displayName(r)}</div>
                    <div className="text-xs text-zinc-500">{r.slug || "no slug"}</div>
                  </td>
                  <td className="text-xs text-zinc-500">{r.method_key || "—"}</td>
                  <td>{r.calculation_type || "fixed"}</td>
                  <td className="text-xs text-zinc-500">{pricingSummary(r)}</td>
                  <td className="text-xs text-zinc-500">{printOptionOutputLabel(r)}</td>
                  <td className="text-xs text-zinc-500">{(r.print_positions || []).join(", ")}</td>
                  <td><StatusBadge status={r.status || "active"} /></td>
                  <td className="text-right whitespace-nowrap">
                    <button onClick={() => startEdit(r)} className="text-xs uppercase tracking-widest text-[#FF3B30] mr-3">Edit</button>
                    <button onClick={() => remove(r)} className="text-xs uppercase tracking-widest text-zinc-500">Delete</button>
                  </td>
                </tr>
              ))}

              {!loadingRows && rows.length === 0 && (
                <tr><td colSpan={8} className="p-10 text-center text-zinc-500 overline">No pricing rules yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function AdminWorkspaceTabs({ tabs, initial = "overview", modules = {}, user = null, mode = "admin" }) {
  const isManager = mode === "manager" || user?.role === "manager";
  const managerPermissions = user?.manager_permissions || {};
  ...
  if (!visibleTabs.length) {
    return <div className="card text-sm text-[var(--ff-muted-text)]">You do not have access to any tabs in this workspace.</div>;
  }
  ...
  return (
    <div data-testid="admin-printers-workspace" className="space-y-6">
      ...
      <AdminWorkspaceTabs
        ...
        mode={mode}
        tabs={[
          { key: "accounts", label: "Accounts", permission: "manage_printers", moduleKey: "printers_enabled", element: <PrintersAdmin /> },
...
function ProductsTemplatesWorkspace({ modules = {}, user = null, mode = "admin" } = {}) {
  return (
    <div data-testid="admin-product-templates-workspace" className="space-y-6">
      ...
      <AdminWorkspaceTabs
        ...
        tabs={[
          ...
          { key: "products", label: "Sellable Products", permission: "manage_products", element: <ProductsAdmin /> },
          ...
          { key: "print-options", label: "Print Options", permission: "manage_product_templates", moduleKey: "product_templates_enabled", element: <PrintOptionsAdmin /> },
        ]}
      />
    </div>
  );
...
function ShopSettingsWorkspace({ modules = {}, user = null, mode = "admin" } = {}) {
  return (
...
function PlatformSettingsWorkspace({ modules = {}, user = null, mode = "admin" } = {}) {
  return (
    <div data-testid="admin-platform-settings-workspace" className="space-y-6">
      ...
      <AdminWorkspaceTabs
        ...
        tabs={[
          ...
          { key: "general", label: "General", ownerOnly: true, element: <AdminSettings initialTab="general" compact /> },
        ]}
      />
    </div>
  );
  ...
  return (
    <Routes>
      <Route element={<DashboardLayout title={title} links={visibleLinks} testidPrefix={mode === "manager" ? "manager-dash" : "admin-dash"} notificationEndpoint="/admin/notifications" notificationPath={`${basePath}/notifications`} />}>
        ...
        <Route path="fulfilment" element={<FulfilmentWorkspace modules={platformConfig?.modules || {}} user={user} mode={mode} />} />
