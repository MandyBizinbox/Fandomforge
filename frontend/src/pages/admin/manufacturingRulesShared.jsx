...
export function Stat({ label, value, icon }) {
  return <div className="card"><div className="flex items-center justify-between gap-3"><div><p className="overline mb-1">{label}</p><div className="font-display text-3xl uppercase">{value}</div></div><div className="text-[var(--ff-primary)]">{icon}</div></div></div>;
}

export function Field({ label, hint, children }) {
  return <label className="block"><span className="label">{label}</span>{children}{hint && <span className="block mt-1 text-xs text-[var(--ff-muted-text)]">{hint}</span>}</label>;
}

export function Toggle({ label, checked, onChange }) {
  return <label className="flex items-center gap-3 text-sm text-[var(--ff-muted-text)]"><input type="checkbox" checked={Boolean(checked)} onChange={(event) => onChange(event.target.checked)} /><span>{label}</span></label>;
}
...
export function MigrationPanel({ status, onReload }) {
  ...
  return <div className={`card border ${complete ? "border-green-500/50" : "border-[var(--ff-primary)]"}`}>
    ...
    {result && <div className="mt-4 grid md:grid-cols-4 gap-3 text-sm">
      {[["Profiles", result.profiles_total], ["Aliases mapped", result.legacy_aliases_mapped], ["References", result.reference_changes], ["Blockers", conflicts.length + unmatched.length]].map(([label, value]) => <div key={label} className={`border p-3 ${label === "Blockers" && value ? "border-red-500" : "border-[var(--ff-card-border)]"}`}><span className="overline">{label}</span><div className="font-display text-2xl">{value || 0}</div></div>)}
      <div className="md:col-span-4 text-xs text-[var(--ff-muted-text)]">Templates: {result.templates_to_update || 0} · Products: {result.products_to_update || 0} · Printer prices: {result.printer_prices_to_update || 0} · Builder drafts: {result.builder_drafts_to_update || 0} · Orders updated: 0{unmatched.length > 0 && <span className="block text-red-400 mt-1">Apply is blocked until {unmatched.length} active legacy option(s) are mapped.</span>}</div>
    </div>}
  </div>;
}
