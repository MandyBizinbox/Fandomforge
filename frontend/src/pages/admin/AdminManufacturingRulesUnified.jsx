...
export default function AdminManufacturingRulesUnified({ activeSection = "methods" }) {
  ...
  return <div className="ff-admin-page"><div className="ff-admin-page__inner">
    <div className="ff-admin-page-header"><div><p className="overline mb-2">Admin / Production</p><h1 className="ff-admin-page-title">Manufacturing Rules</h1><p className="ff-admin-page-description">One engine for method behaviour, costing profiles, stocked-colour restrictions and production validation.</p></div><div className="flex gap-2"><Link to="/admin" className="ff-admin-button ff-admin-button--secondary">Back to Admin</Link><button type="button" className="ff-admin-button ff-admin-button--secondary flex items-center gap-2" onClick={load}><RefreshCw size={14} /> Refresh</button></div></div>

    <div className="ff-admin-stat-grid"><Stat label="Print methods" value={methods.length} icon={<Factory />} /><Stat label="Profiles" value={profileCount} icon={<Settings />} /><Stat label="Stock colours" value={colours.length} icon={<Palette />} /><Stat label="Restricted" value={restrictedCount} icon={<ShieldCheck />} /></div>
    ...
    {activeSection === "colours" && <div className="space-y-4"><div className="ff-admin-card space-y-4"><div><p className="overline mb-1">Create colour</p><h2 className="font-display text-3xl uppercase">Add stocked colour</h2></div><div className="grid md:grid-cols-[1fr,150px,1fr,120px] gap-4 items-end"><Field label="Name"><input className="ff-admin-control" value={newColour.name} onChange={(event) => setNewColour((current) => ({ ...current, name: event.target.value }))} /></Field><Field label="Hex"><input className="ff-admin-control" type="color" value={newColour.hex} onChange={(event) => setNewColour((current) => ({ ...current, hex: event.target.value }))} /></Field><Field label="Applies to methods"><textarea className="ff-admin-control min-h-[60px]" value={newColour.appliesToMethodsText} onChange={(event) => setNewColour((current) => ({ ...current, appliesToMethodsText: event.target.value }))} /></Field><button type="button" className="ff-admin-button ff-admin-button--primary h-12" disabled={saving} onClick={createColour}>Create</button></div></div>
      <div className="grid lg:grid-cols-2 gap-4">{colours.map((colour) => { const draft = colourDrafts[colour.id] || colourDraft(colour); return <div key={colour.id} className={`ff-admin-card space-y-4 ${draft.active === false ? "opacity-60" : ""}`}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="w-10 h-10 ff-admin-swatch" style={{ background: draft.hex }} /><div><p className="overline">{colour.id}</p><h3 className="font-display text-2xl uppercase">{draft.name}</h3></div></div><button type="button" className="ff-admin-button ff-admin-button--secondary flex items-center gap-2" disabled={saving} onClick={() => saveColour(colour.id)}><Save size={14} /> Save</button></div><div className="grid md:grid-cols-2 gap-4"><Field label="Name"><input className="ff-admin-control" value={draft.name || ""} onChange={(event) => patchColour(colour.id, { name: event.target.value })} /></Field><Field label="Hex"><input className="ff-admin-control" type="color" value={draft.hex || "#000000"} onChange={(event) => patchColour(colour.id, { hex: event.target.value })} /></Field><NumericField label="Sort order" value={draft.sort_order} onChange={(value) => patchColour(colour.id, { sort_order: value })} /><div className="pt-7"><Toggle label="Colour active" checked={draft.active !== false} onChange={(active) => patchColour(colour.id, { active })} /></div><Field label="Aliases"><textarea className="ff-admin-control min-h-[72px]" value={draft.aliasesText || ""} onChange={(event) => patchColour(colour.id, { aliasesText: event.target.value })} /></Field><Field label="Applies to methods"><textarea className="ff-admin-control min-h-[72px]" value={draft.appliesToMethodsText || ""} onChange={(event) => patchColour(colour.id, { appliesToMethodsText: event.target.value })} /></Field></div></div>; })}</div>
    </div>}

    {activeSection === "settings" && <div className="ff-admin-card space-y-5">{!settings ? <p className="text-sm ff-admin-muted">Global settings require super-admin access.</p> : <><div className="flex items-start justify-between gap-4"><div><p className="overline mb-1">Global costing</p><h2 className="font-display text-3xl uppercase">Platform controls</h2><p className="text-sm ff-admin-muted mt-2">Print production pricing belongs to method profiles. These controls remain global.</p></div><button type="button" className="ff-admin-button ff-admin-button--primary flex items-center gap-2" disabled={saving} onClick={saveSettings}><Save size={14} /> Save settings</button></div><div className="grid md:grid-cols-3 gap-4"><NumericField label="Default packaging cost" value={settings.default_packaging_cost} onChange={(value) => setSettings((current) => ({ ...current, default_packaging_cost: value }))} /><NumericField label="Packaging creator markup %" value={settings.default_packaging_creator_markup_percent} onChange={(value) => setSettings((current) => ({ ...current, default_packaging_creator_markup_percent: value }))} /><NumericField label="Minimum creator profit" value={settings.minimum_creator_profit_required} onChange={(value) => setSettings((current) => ({ ...current, minimum_creator_profit_required: value }))} /></div><div className="grid md:grid-cols-3 gap-4"><Toggle label="Block publish on warnings" checked={settings.fail_publish_on_warnings} onChange={(value) => setSettings((current) => ({ ...current, fail_publish_on_warnings: value }))} /><Toggle label="Allow unknown material warning" checked={settings.allow_unknown_material_with_warning} onChange={(value) => setSettings((current) => ({ ...current, allow_unknown_material_with_warning: value }))} /><Toggle label="Allow unknown category warning" checked={settings.allow_unknown_category_with_warning} onChange={(value) => setSettings((current) => ({ ...current, allow_unknown_category_with_warning: value }))} /></div><Field label="Notes"><textarea className="ff-admin-control min-h-[90px]" value={settings.notes || ""} onChange={(event) => setSettings((current) => ({ ...current, notes: event.target.value }))} /></Field></>}</div>}
  </div></div>;
}
