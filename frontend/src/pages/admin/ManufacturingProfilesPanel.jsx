...
function ProfileEditor({ profile, onPatch, onDefault, onDuplicate, onArchive }) {
  ...
  return <div className={`border p-4 space-y-4 ${profile.is_default ? "border-[var(--ff-primary)]" : "border-[var(--ff-card-border)]"} ${profile.status === "archived" ? "opacity-60" : ""}`}>
    <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
      <div className="flex-1"><div className="flex flex-wrap items-center gap-2 mb-2">{profile.is_default && <span className="text-[10px] uppercase tracking-widest border border-[var(--ff-primary)] px-2 py-1">Default</span>}<span className="text-[10px] uppercase tracking-widest text-[var(--ff-muted-text)]">{profile.status}</span>{safeArray(profile.legacy_print_option_ids).length > 0 && <span className="text-[10px] uppercase tracking-widest text-[var(--ff-muted-text)]">{profile.legacy_print_option_ids.length} aliases</span>}</div><input className="input-base font-display text-2xl uppercase" value={profile.display_name || profileName(profile)} onChange={(event) => onPatch({ display_name: event.target.value, profile_name: event.target.value })} /><p className="text-xs text-[var(--ff-muted-text)] mt-2">{profileSummary(profile)}</p></div>
      <div className="flex flex-wrap gap-2">{!profile.is_default && profile.status !== "archived" && <button type="button" className="btn-secondary flex items-center gap-2" onClick={onDefault}><Star size={14} /> Set default</button>}<button type="button" className="btn-secondary flex items-center gap-2" onClick={onDuplicate}><Copy size={14} /> Duplicate</button>{profile.status !== "archived" && <button type="button" className="btn-secondary flex items-center gap-2" onClick={onArchive}><Archive size={14} /> Archive</button>}</div>
    </div>
    ...
    {advancedOpen && <div className="border-t border-[var(--ff-card-border)] pt-4 space-y-4">
      <div className="grid md:grid-cols-4 gap-4"><NumericField label="Waste %" value={profile.waste_percentage} onChange={(value) => onPatch({ waste_percentage: value })} /><NumericField label="Markup %" value={profile.markup_percentage} onChange={(value) => onPatch({ markup_percentage: value })} /><NumericField label="Minimum print cost" hint="Exceptional monetary floor. The normal outsourced model uses minimum area instead." value={profile.minimum_print_cost} onChange={(value) => onPatch({ minimum_print_cost: value })} /><Field label="Status"><select className="input-base" value={profile.status || "active"} onChange={(event) => onPatch({ status: event.target.value })}><option value="active">Active</option><option value="inactive">Inactive</option><option value="draft">Draft</option><option value="archived">Archived</option></select></Field></div>
      <div className="grid md:grid-cols-3 gap-4"><Field label="Markup type"><select className="input-base" value={profile.platform_print_markup_type || "manual"} onChange={(event) => onPatch({ platform_print_markup_type: event.target.value })}><option value="manual">Manual</option><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option></select></Field><NumericField label="Markup value" value={profile.platform_print_markup_value} onChange={(value) => onPatch({ platform_print_markup_value: value })} /><Field label="Legacy aliases" hint="Compatibility only. One per line."><textarea className="input-base min-h-[90px]" value={safeArray(profile.legacy_print_option_ids).join("\n")} onChange={(event) => onPatch({ legacy_print_option_ids: event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean) })} /></Field></div>
      <div className="grid md:grid-cols-2 gap-4"><Field label="Placement tags"><textarea className="input-base min-h-[90px]" value={safeArray(profile.print_positions || profile.placement_tags).join("\n")} onChange={(event) => onPatch({ print_positions: event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean), placement_tags: event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean) })} /></Field><Field label="Pricing notes"><textarea className="input-base min-h-[90px]" value={profile.pricing_notes || ""} onChange={(event) => onPatch({ pricing_notes: event.target.value })} /></Field></div>
    </div>}
  </div>;
}
...
function MethodBehaviour({ method, colours, onPatch }) {
  ...
  return <div className="card"><button type="button" className="w-full flex items-center justify-between" onClick={() => onPatch({ _behaviourOpen: !expanded })}><div className="text-left"><p className="overline mb-1">Production behaviour</p><h3 className="font-display text-2xl uppercase">Method constraints</h3></div>{expanded ? <ChevronUp /> : <ChevronDown />}</button>{expanded && <div className="mt-5 space-y-5">
    ...
    <div className="grid md:grid-cols-2 gap-4"><Field label="Supported categories"><textarea className="input-base min-h-[100px]" value={method.categoriesText || ""} onChange={(event) => onPatch({ categoriesText: event.target.value })} /></Field><Field label="Supported materials"><textarea className="input-base min-h-[100px]" value={method.materialsText || ""} onChange={(event) => onPatch({ materialsText: event.target.value })} /></Field><Field label="Supported artwork types"><textarea className="input-base min-h-[90px]" value={method.artworkTypesText || ""} onChange={(event) => onPatch({ artworkTypesText: event.target.value })} /></Field><Field label="Stocked colour restriction"><div className="grid grid-cols-2 gap-2 max-h-[140px] overflow-auto border border-[var(--ff-card-border)] p-3">{colours.map((colour) => <Toggle key={colour.id} label={colour.name} checked={safeArray(method.selectedColourIds).includes(colour.id)} onChange={(checked) => onPatch({ selectedColourIds: checked ? [...safeArray(method.selectedColourIds), colour.id] : safeArray(method.selectedColourIds).filter((id) => id !== colour.id) })} />)}</div></Field></div>
  </div>}</div>;
}
...
export default function ManufacturingProfilesPanel({ methods, selectedKey, onSelect, colours, onPatchMethod, onPatchProfile, onDefault, onDuplicate, onArchive, onAdd, onSave, saving }) {
  ...
  return <div className="grid lg:grid-cols-[280px,1fr] gap-4">
    <div className="card space-y-2"><p className="overline mb-3">Production methods</p>{methods.map((method) => <button type="button" key={method.method_key} onClick={() => onSelect(method.method_key)} className={`w-full border p-3 text-left ${selected?.method_key === method.method_key ? "border-[var(--ff-primary)] bg-[var(--ff-primary)] text-white" : "border-[var(--ff-card-border)]"}`}><div className="font-display text-xl uppercase">{method.display_name || method.method_key}</div><div className="text-xs uppercase tracking-widest mt-1">{safeArray(method.profiles).length} profiles</div></button>)}</div>
    {selected && <div className="space-y-4"><div className="card"><div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4"><div><p className="overline mb-2">Selected method</p><h2 className="font-display text-4xl uppercase">{selected.display_name}</h2><p className="text-sm text-[var(--ff-muted-text)] mt-2">One canonical profile collection. The default is used unless a product or print area selects a specialist profile.</p></div><button type="button" onClick={() => onSave(selected.method_key)} disabled={saving} className="btn-primary flex items-center gap-2"><Save size={15} /> Save method & profiles</button></div></div>
      <div className="card space-y-4"><div className="flex items-center justify-between"><div><p className="overline mb-1">Costing profiles</p><h3 className="font-display text-3xl uppercase">Per-profile pricing</h3></div><button type="button" onClick={() => onAdd(selected.method_key)} className="btn-secondary flex items-center gap-2"><Plus size={14} /> Add profile</button></div>{safeArray(selected.profiles).map((profile, index) => <ProfileEditor key={profile.id || index} profile={profile} onPatch={(updates) => onPatchProfile(selected.method_key, index, updates)} onDefault={() => onDefault(selected.method_key, index)} onDuplicate={() => onDuplicate(selected.method_key, index)} onArchive={() => onArchive(selected.method_key, index)} />)}{!safeArray(selected.profiles).length && <div className="border border-dashed border-[var(--ff-card-border)] p-5 text-sm text-[var(--ff-muted-text)]">No profiles yet. Add the first costing profile.</div>}</div>
