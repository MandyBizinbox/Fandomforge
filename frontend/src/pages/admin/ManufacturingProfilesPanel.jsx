...
function ProfileColourSelector({ profile, method, colours, onPatch }) {
  ...
  return <div className="ff-admin-subpanel space-y-3">
    <div>
      ...
      <p className="text-xs ff-admin-muted mt-1">
    ...
    {restricted && <>
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-2 max-h-[240px] overflow-auto border border-[var(--ff-card-border)] p-3">
        {availableColours.map((colour) => <label key={colour.id} className="flex items-center gap-2 border border-[var(--ff-card-border)] p-2 text-sm cursor-pointer">
          ...
          />
          <span className="w-5 h-5 ff-admin-swatch shrink-0" style={{ background: colour.hex || "transparent" }} />
...
function ProfileEditor({ profile, method, colours, onPatch, onDefault, onDuplicate, onArchive }) {
  ...
  return <div className={`ff-admin-subpanel space-y-4 ${profile.is_default ? "ff-admin-primary-border" : ""} ${profile.status === "archived" ? "opacity-60" : ""}`}>
    <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
      <div className="flex-1">
        <div className="flex flex-wrap items-center gap-2 mb-2">
          {profile.is_default && <span className="text-[10px] uppercase tracking-widest border ff-admin-primary-border px-2 py-1">Default</span>}
          <span className="text-[10px] uppercase tracking-widest ff-admin-muted">{profile.status}</span>
          {safeArray(profile.legacy_print_option_ids).length > 0 && <span className="text-[10px] uppercase tracking-widest ff-admin-muted">{profile.legacy_print_option_ids.length} aliases</span>}
        </div>
        <input className="ff-admin-control font-display text-2xl uppercase" value={profile.display_name || profileName(profile)} onChange={(event) => onPatch({ display_name: event.target.value, profile_name: event.target.value })} />
        <p className="text-xs ff-admin-muted mt-2">{profileSummary(profile)}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        {!profile.is_default && profile.status !== "archived" && <button type="button" className="ff-admin-button ff-admin-button--secondary flex items-center gap-2" onClick={onDefault}><Star size={14} /> Set default</button>}
        <button type="button" className="ff-admin-button ff-admin-button--secondary flex items-center gap-2" onClick={onDuplicate}><Copy size={14} /> Duplicate</button>
        {profile.status !== "archived" && <button type="button" className="ff-admin-button ff-admin-button--secondary flex items-center gap-2" onClick={onArchive}><Archive size={14} /> Archive</button>}
      </div>
    </div>

    <div className="grid md:grid-cols-4 gap-4">
      <Field label="Calculation type"><select className="ff-admin-control" value={profile.calculation_type || "area_fixed_rate"} onChange={(event) => onPatch({ calculation_type: event.target.value })}>{CALCULATION_TYPES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></Field>
    ...
    {advancedOpen && <div className="border-t border-[var(--ff-card-border)] pt-4 space-y-4">
      <div className="grid md:grid-cols-4 gap-4">
        ...
        <Field label="Status"><select className="ff-admin-control" value={profile.status || "active"} onChange={(event) => onPatch({ status: event.target.value })}><option value="active">Active</option><option value="inactive">Inactive</option><option value="draft">Draft</option><option value="archived">Archived</option></select></Field>
      </div>
      <div className="grid md:grid-cols-3 gap-4">
        <Field label="Markup type"><select className="ff-admin-control" value={profile.platform_print_markup_type || "manual"} onChange={(event) => onPatch({ platform_print_markup_type: event.target.value })}><option value="manual">Manual</option><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option></select></Field>
        ...
        <Field label="Legacy aliases" hint="Compatibility only. One per line."><textarea className="ff-admin-control min-h-[90px]" value={safeArray(profile.legacy_print_option_ids).join("\n")} onChange={(event) => onPatch({ legacy_print_option_ids: event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean) })} /></Field>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <Field label="Placement tags"><textarea className="ff-admin-control min-h-[90px]" value={safeArray(profile.print_positions || profile.placement_tags).join("\n")} onChange={(event) => onPatch({ print_positions: event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean), placement_tags: event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean) })} /></Field>
        <Field label="Pricing notes"><textarea className="ff-admin-control min-h-[90px]" value={profile.pricing_notes || ""} onChange={(event) => onPatch({ pricing_notes: event.target.value })} /></Field>
      </div>
    </div>}
  </div>;
}
...
export default function ManufacturingProfilesPanel({ methods, selectedKey, onSelect, colours, onPatchMethod, onPatchProfile, onDefault, onDuplicate, onArchive, onAdd, onSave, saving }) {
  ...
  return <div className="grid lg:grid-cols-[280px,1fr] gap-4">
    ...
    {selected && <div className="space-y-4">
      <div className="ff-admin-card"><div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4"><div><p className="overline mb-2">Selected method</p><h2 className="font-display text-4xl uppercase">{selected.display_name}</h2><p className="text-sm ff-admin-muted mt-2">One canonical profile collection. The default is used unless a product or print area selects a specialist profile.</p></div><button type="button" onClick={() => onSave(selected.method_key)} disabled={saving} className="ff-admin-button ff-admin-button--primary flex items-center gap-2"><Save size={15} /> Save method & profiles</button></div></div>
      <div className="ff-admin-card space-y-4"><div className="flex items-center justify-between"><div><p className="overline mb-1">Costing profiles</p><h3 className="font-display text-3xl uppercase">Per-profile pricing</h3></div><button type="button" onClick={() => onAdd(selected.method_key)} className="ff-admin-button ff-admin-button--secondary flex items-center gap-2"><Plus size={14} /> Add profile</button></div>{safeArray(selected.profiles).map((profile, index) => <ProfileEditor key={profile.id || index} profile={profile} method={selected} colours={colours} onPatch={(updates) => onPatchProfile(selected.method_key, index, updates)} onDefault={() => onDefault(selected.method_key, index)} onDuplicate={() => onDuplicate(selected.method_key, index)} onArchive={() => onArchive(selected.method_key, index)} />)}{!safeArray(selected.profiles).length && <div className="border border-dashed border-[var(--ff-card-border)] p-5 text-sm ff-admin-muted">No profiles yet. Add the first costing profile.</div>}</div>
