...
import { composeAttributeGeometryPreview } from "./attributeProductionComposition";
...
export function normaliseProductionConfiguration(value = {}) {
  ...
  const screens = activeRows(source.screens || source.mockup_screens)
    .map((screen, index) => ({
      ...clone(screen),
  ...
  const optionIds = new Set([
    ...
    ...safeArray(source.print_options)
...
export function getVariationProductionConfiguration(
  variation = {},
  template = {}
) {
  const stored = variation?.print_area_overrides?.[PRODUCTION_CONFIG_KEY];
  if (stored && typeof stored === "object") {
    return normaliseProductionConfiguration(stored);
  }
...
export function setVariationProductionConfiguration(
  variation = {},
  configuration = {}
) {
...
export function getAttributeProfileConfiguration(
  profiles = {},
  value = ""
) {
  ...
  const fallback = Object.values(rows).find(
    (row) => (
      normaliseKey(row?.attribute_value) === normaliseKey(value)
    )
  );
  ...
  return Array.from(
    new Set(
      activeVariationRows(variations)
        .map(
          (variation) => getVariationAttributeValue(
            variation,
            attributeName
          )
        )
...
export function resolveVariationProductionConfiguration(
  ...
  const effectiveImageConfiguration = (
    safeArray(imageConfiguration.screens).length
      ? imageConfiguration
      : base
  );
...
export function applyProductionConfigurationToVariations(
  variations = [],
  configuration = {},
  variationIds = null
) {
  ...
  return safeArray(variations).map((variation) => {
    ...
    return setVariationProductionConfiguration(
      variation,
      cloneProductionConfiguration(source)
    );
  });
}
...
function screenSlotRows(config = {}) {
  return withOccurrenceKeys(
    normaliseProductionConfiguration(config).screens,
    screenIdentity
  );
}
...
function areaSlotRows(config = {}) {
  ...
  const screenSlotById = new Map(
    screenSlots.map(({ row, slotKey }) => [row.id, slotKey])
  );
  const counts = new Map();

  return normalised.print_areas.map((area) => {
    const screenSlot = (
      screenSlotById.get(area.screen_id)
      || screenIdentity({
        view_key: area.view_key || area.screen_view,
      })
    );
  ...
  configurations.forEach((config) => {
    safeArray(config.print_options).forEach((option) => {
      if (option?.id && !byId.has(option.id)) {
        byId.set(option.id, clone(option));
      }
    });
  });
...
function compiledVariationConfiguration(
  ...
  const configScreens = new Map(
    screenSlotRows(config).map(
      (entry) => [entry.slotKey, entry.row]
    )
  );
  const configAreas = new Map(
    areaSlotRows(config).map(
      (entry) => [entry.slotKey, entry.row]
    )
  );
  ...
  const printAreas = areaAnchors
    .map((anchor) => {
      ...
      return {
        ...clone(source),
        id: anchor.id,
        screen_id: screenIds.has(anchor.screen_id)
          ? anchor.screen_id
          : screens[0]?.id || anchor.screen_id,
        name: source.name || anchor.name,
        status: "active",
      };
    })
...
export function compileVariableTemplateProduction(
  template = {},
  variations = []
) {
  const activeVariations = safeArray(variations).filter(
    (variation) => variation
      && variation.enabled !== false
      && variation.status !== "archived"
  );
  ...
  const rows = activeVariations.map((variation) => ({
    variation,
    config: resolveVariationProductionConfiguration(
      variation,
      template
    ),
  }));
  ...
  rows.forEach(({ config }) => {
    screenSlotRows(config).forEach(({ row, slotKey }) => {
      if (!screenRepresentatives.has(slotKey)) {
        screenRepresentatives.set(slotKey, row);
      }
    });
    areaSlotRows(config).forEach(({ row, slotKey, screenSlot }) => {
      if (!areaRepresentatives.has(slotKey)) {
        areaRepresentatives.set(slotKey, { row, screenSlot });
      }
    });
  });

  const screenAnchors = Array.from(screenRepresentatives.entries())
    .map(([slotKey, representative], index) => ({
  ...
  const screenAnchorBySlot = new Map(
    screenAnchors.map(
      (screen) => [screen.production_slot_key, screen]
    )
  );

  const areaAnchors = Array.from(areaRepresentatives.entries())
    .map(([slotKey, entry], index) => {
      const screenAnchor = (
        screenAnchorBySlot.get(entry.screenSlot)
        || screenAnchors[0]
      );
      return {
        ...clone(entry.row),
  ...
  const compiledVariations = safeArray(variations).map((variation) => {
    if (!activeVariations.some((active) => active.id === variation.id)) {
      return variation;
    }

    const sourceConfig = resolveVariationProductionConfiguration(
      variation,
      template
    );
    ...
    const configScreensById = new Map(
      config.screens.map((screen) => [screen.id, screen])
    );
    const configAreasById = new Map(
      config.print_areas.map((area) => [area.id, area])
    );
  ...
  const optionIds = new Set([
    ...configurations.flatMap(
      (config) => safeArray(config.print_option_ids)
    ),
    ...areaAnchors.flatMap(
      (area) => safeArray(area.allowed_print_option_ids)
    ),
  ...
  return {
    ...
    mockup_screens: screenAnchors.map(
      ({ production_slot_key, ...screen }) => screen
    ),
    print_areas: areaAnchors.map(
      ({ production_slot_key, ...area }) => area
    ),
    print_option_ids: Array.from(optionIds),
...
export function variationProductionSummary(
  variation = {},
  template = {}
) {
  const config = resolveVariationProductionConfiguration(
    variation,
    template
  );
  return {
