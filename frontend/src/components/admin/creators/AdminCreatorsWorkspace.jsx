...
function AssetUploadField({ label, value, onChange, subdir = "account-assets" }) {
  ...
  return (
    <div className="ff-admin-subpanel p-3">
      <label className="ff-admin-label">{label}</label>
      <div className="aspect-[4/3] ff-admin-subpanel overflow-hidden flex items-center justify-center mb-3">
        {value ? (
          <img src={assetUrl(value)} alt={label} className="w-full h-full object-contain" />
        ) : (
          <div className="text-xs ff-admin-muted uppercase tracking-widest">No image</div>
        )}
      </div>
      <div className="flex gap-2">
        <label className="ff-admin-button ff-admin-button--secondary text-xs flex-1 justify-center cursor-pointer">
          {uploading ? "Uploading…" : value ? "Replace" : "Upload"}
          <input type="file" accept="image/*" className="hidden" onChange={(event) => upload(event.target.files?.[0])} />
        </label>
        {value && (
          <button type="button" className="border border-[var(--ff-card-border)] px-3 text-xs uppercase tracking-widest ff-admin-muted hover:text-[var(--ff-primary)]" onClick={() => onChange("")}>
            Clear
          </button>
        )}
      </div>
  ...
  return (
    <div data-testid="admin-creators-page" className="ff-admin-page"><div className="ff-admin-page__inner">
      {view === "list" ? (
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="ff-admin-page-title">Creators</h1>
            <p className="ff-admin-page-description">Manage creator storefront accounts. Open a creator to edit its profile, publishing, billing and ownership settings.</p>
          </div>
          <button type="button" onClick={() => navigate(`${basePath}/creators/accounts/new`)} className="ff-admin-button ff-admin-button--primary"><Plus size={14} /> New Creator</button>
        </div>
      ) : (
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <button type="button" onClick={goToList} className="text-xs uppercase tracking-widest ff-admin-muted hover:text-[var(--ff-card-text)] font-bold mb-4">← Back to Creators</button>
            <div className="overline mb-2">Creator Account</div>
            <h1 className="ff-admin-page-title">{view === "edit" ? (form.name || "Edit Creator") : "New Creator"}</h1>
            <p className="ff-admin-page-description">{view === "edit" ? "Update this creator account and storefront settings." : "Create a new top-level creator storefront account."}</p>
          </div>
        </div>
      )}

      {view !== "list" && (
      <form onSubmit={save} className="ff-admin-card grid lg:grid-cols-3 gap-4">
        ...
        <label><span className="ff-admin-label">Name</span><input className="ff-admin-control" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
        <label><span className="ff-admin-label">Slug</span><input className="ff-admin-control" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="auto-from-name if blank" /></label>
        <label><span className="ff-admin-label">Category</span><input className="ff-admin-control" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Band, Club, School, Scout Group" /></label>

        <label><span className="ff-admin-label">Contact email</span><input className="ff-admin-control" value={form.contact_email} onChange={(e) => setForm({ ...form, contact_email: e.target.value })} /></label>
        <label><span className="ff-admin-label">Contact phone</span><input className="ff-admin-control" value={form.contact_phone} onChange={(e) => setForm({ ...form, contact_phone: e.target.value })} /></label>
        <label><span className="ff-admin-label">Website URL</span><input className="ff-admin-control" value={form.website_url} onChange={(e) => setForm({ ...form, website_url: e.target.value })} /></label>

        <label className="lg:col-span-3"><span className="ff-admin-label">Bio / public description</span><textarea className="ff-admin-control" rows={4} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} /></label>
        ...
        <div className="lg:col-span-3 ff-admin-subpanel p-4 space-y-4">
          <div>
            ...
            <p className="text-sm ff-admin-muted mt-1">Unlisted is recommended for Scout groups, schools, churches, clubs, and private fundraising campaigns.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-4">
            <label>
              <span className="ff-admin-label">Store Visibility</span>
              <select className="ff-admin-control" value={form.visibility} onChange={(e) => setForm({ ...form, visibility: e.target.value })}>
                {CREATOR_VISIBILITY_OPTIONS.map((option) => (
                  <option key={option.value} value={option.value}>{option.label}</option>
                ))}
              </select>
              {form.visibility === "private" && (
                <p className="text-xs ff-admin-muted mt-1">Private stores are reserved for future restricted-access workflows.</p>
              )}
            </label>

            <label className="md:col-span-2">
              <span className="ff-admin-label">Gallery Display Name</span>
              <input className="ff-admin-control" value={form.gallery_display_name} onChange={(e) => setForm({ ...form, gallery_display_name: e.target.value })} placeholder="Falls back to creator name" />
            </label>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <label className="ff-admin-subpanel flex items-start gap-3 p-3 cursor-pointer">
              ...
              <span>
                ...
                <span className="block text-xs ff-admin-muted mt-1">Homepage gallery display is social proof only. Logos/banners are not linked to creator stores.</span>
              </span>
            </label>
            ...
            <label className="ff-admin-subpanel flex items-start gap-3 p-3 cursor-pointer">
              ...
              <span>
                ...
                <span className="block text-xs ff-admin-muted mt-1">Unlisted and private store pages remain noindex unless this is explicitly enabled for a public store.</span>
              </span>
            </label>
          </div>
        </div>
        ...
        <label>
          <span className="ff-admin-label">Platform Commission %</span>
          <input
            className="ff-admin-control"
            type="number"
            step="0.01"
            min="0"
            max="100"
            ...
            placeholder="15"
          />
          <span className="block text-xs ff-admin-muted mt-1">Leave blank to use the platform default commission. Use this for discounted creator packages or custom agreements.</span>
        </label>
        <label className="ff-admin-subpanel flex items-start gap-3 p-3 cursor-pointer">
          ...
          <span>
            ...
            <span className="block text-xs ff-admin-muted mt-1">Optional label only; commission still comes from Platform Commission %.</span>
          </span>
        </label>
        <label><span className="ff-admin-label">Monthly package name</span><input className="ff-admin-control" value={form.monthly_package_name} onChange={(e) => setForm({ ...form, monthly_package_name: e.target.value })} placeholder="Optional package name" /></label>
        <label><span className="ff-admin-label">Monthly fee</span><input className="ff-admin-control" type="number" step="0.01" value={form.monthly_fee} onChange={(e) => setForm({ ...form, monthly_fee: e.target.value })} /></label>
        <label><span className="ff-admin-label">Linked owner user</span><select className="ff-admin-control" value={form.user_id} onChange={(e) => setForm({ ...form, user_id: e.target.value })}><option value="">No linked user</option>{users.map((u) => <option key={u.id} value={u.id}>{u.name || u.email} · {u.email}</option>)}</select></label>

        <label><span className="ff-admin-label">Subscription</span><select className="ff-admin-control" value={form.subscription_status} onChange={(e) => setForm({ ...form, subscription_status: e.target.value })}><option value="active">Active</option><option value="inactive">Inactive</option><option value="past_due">Past due</option></select></label>
        <label><span className="ff-admin-label">Status</span><select className="ff-admin-control" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="active">Active</option><option value="pending">Pending</option><option value="suspended">Suspended</option><option value="archived">Archived</option></select></label>
        <label><span className="ff-admin-label">Socials JSON</span><textarea className="ff-admin-control" rows={3} value={form.socials_text} onChange={(e) => setForm({ ...form, socials_text: e.target.value })} placeholder={'{"facebook":"https://..."}'} /></label>

        <div className="lg:col-span-3 flex gap-3">
          <button type="submit" className="ff-admin-button ff-admin-button--primary" disabled={saving}><Save size={14} /> {saving ? "Saving…" : view === "edit" ? "Update Creator" : "Create Creator"}</button>
          <button type="button" onClick={goToList} className="ff-admin-button ff-admin-button--secondary">Cancel</button>
        </div>
      </form>
      )}
      ...
      <div className="ff-admin-card p-0 overflow-x-auto">
        <table className="table-brutal min-w-[1100px] w-full">
          ...
          <tbody>
            {rows.map((b) => (
              <tr key={b.id} data-testid={`admin-creator-row-${b.id}`}>
                <td><div className="font-bold">{b.name}</div><div className="text-xs ff-admin-muted">{b.category || "No category"} · {b.contact_email || "No email"}</div></td>
                <td className="ff-admin-muted">/{b.slug}</td>
                <td className="text-xs ff-admin-muted">{b.user_id || "No linked owner"}</td>
                <td><StatusBadge status={b.status} /></td>
                <td className="text-xs ff-admin-muted">
                ...
                <td><select className="ff-admin-control py-1 text-xs" defaultValue="" onChange={(e) => linkUser(b, e.target.value)}><option value="">Link user</option>{users.map((u) => <option key={u.id} value={u.id}>{u.name || u.email}</option>)}</select></td>
                <td className="text-right whitespace-nowrap"><button type="button" onClick={() => navigate(`${basePath}/creators/accounts/${b.id}`)} className="text-xs uppercase tracking-widest text-[var(--ff-primary)] font-bold mr-4">Edit</button><button type="button" onClick={() => remove(b)} className="text-xs uppercase tracking-widest ff-admin-muted hover:text-[var(--ff-primary)] font-bold">Delete</button></td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={9} className="p-10 text-center ff-admin-muted overline">No creators yet</td></tr>}
          </tbody>
        </table>
      </div>
      )}
  ...
  return (
    <div data-testid="admin-products-page">
      <div className="flex items-center justify-between mb-8">
        <h1 className="ff-admin-page-title">Products</h1>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => navigate("/admin/simple-products/new")} className="ff-admin-button ff-admin-button--secondary"><Plus size={14} /> New Simple Product</button>
          <button onClick={() => navigate("/admin/products/new")} className="ff-admin-button ff-admin-button--primary"><Plus size={14} /> New Template Product</button>
        </div>
      </div>
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={p.id} data-testid={`admin-product-row-${p.id}`}>
                ...
                <td className="text-right"><button onClick={() => navigate(`/admin/products/${p.id}`)} className="text-xs uppercase tracking-widest text-[var(--ff-primary)] hover:text-[var(--ff-card-text)] font-bold mr-3">Edit</button><button onClick={() => remove(p)} className="text-xs uppercase tracking-widest ff-admin-muted hover:text-[var(--ff-primary)] font-bold">Delete</button></td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan={5} className="p-10 text-center ff-admin-muted overline">No products</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
  ...
  return (
    <div data-testid="admin-creators-workspace-routed" className="space-y-6">
      <div>
        ...
        <h1 className="ff-admin-page-title">Creators</h1>
        <p className="ff-admin-page-description">Creator accounts, users, products, artwork review, subscriptions and payouts now own concrete routes while remaining API-backed.</p>
      </div>

      {!!tabs.length && (
        <nav className="flex flex-wrap gap-2 border-b border-[var(--ff-card-border)] pb-4">
          {tabs.map((tab) => (
            <NavLink key={tab.to} to={tab.to} className={({ isActive }) => `px-4 py-3 border text-xs uppercase tracking-widest font-bold ${isActive ? "border-[var(--ff-primary)] bg-[var(--ff-primary)] text-[var(--ff-card-text)]" : "border-[var(--ff-card-border)] ff-admin-muted hover:text-[var(--ff-card-text)]"}`}>
              {tab.label}
            </NavLink>
          ))}
        </nav>
