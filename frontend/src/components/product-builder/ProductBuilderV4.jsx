...
function BasicsStep({ form, update, productTypes, selectedProductTypeId, chooseType, templates, selectedTemplate, chooseTemplate, creators, isAdmin, product }) {
  return <div className="space-y-6"><section className="card space-y-5"><div><div className="overline mb-1">Product basics</div><p className="text-sm pb4-muted">Choose the template here. Its production specs are loaded into the editable Specs box below.</p></div>
    ...
    <Field label="Product type"><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{productTypes.map((type) => <button key={type.id} type="button" onClick={() => chooseType(type.id)} className={`pb4-choice-card ${selectedProductTypeId === type.id ? "is-selected" : ""}`}><div className="font-bold text-[var(--ff-card-text)]">{type.name}</div><div className="text-xs pb4-muted mt-1">{type.category || type.slug}</div></button>)}</div></Field>
    <Field label="Template"><div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">{templates.map((template) => { const selected = selectedTemplate?.id === template.id; const image = getTemplateImage(template); return <button key={template.id} type="button" onClick={() => chooseTemplate(template)} className={`pb4-choice-card ${selected ? "is-selected" : ""}`}><div className="flex gap-3"><div className="pb4-choice-image">{image ? <img src={assetUrl(image)} alt={template.name} className="w-full h-full object-contain" /> : <Package size={24} className="pb4-muted" />}</div><div className="min-w-0"><div className="font-display text-xl uppercase">{template.name}</div><div className="text-xs pb4-muted mt-1">{getTemplateShortDescription(template)}</div>{selected && <div className="pb4-selected-label">Selected</div>}</div></div><div className="grid grid-cols-2 gap-2 mt-4"><Info label="Base" value={money(getCreatorBlankPrice(template))} /><Info label="Print areas" value={String(asArray(template.print_areas).length)} /></div></button>; })}</div>{!templates.length && <div className="text-sm pb4-muted">Select a product type to see compatible templates.</div>}</Field>
    ...
    {selectedTemplate && <div className="pb4-step-summary"><div className="overline mb-2">Template production summary</div><div className="grid sm:grid-cols-3 gap-3"><Info label="Options" value={getTemplateAvailableOptionsSummary(selectedTemplate).join(" · ") || "Configured"} /><Info label="Print areas" value={String(asArray(selectedTemplate.print_areas).length)} /><Info label="Production methods" value={String(asArray(selectedTemplate.production_methods || selectedTemplate.print_options).length || "Configured")} /></div></div>}
