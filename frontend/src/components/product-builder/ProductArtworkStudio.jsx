...
export default function ProductArtworkStudio({
  ...
  const printAreas = useMemo(() => {
    return asArray(template?.print_areas)
      ...
      .map((area) => ({
        ...area,
        x_pct: area.x_pct ?? area.x ?? 0,
        y_pct: area.y_pct ?? area.y ?? 0,
        width_pct: area.width_pct ?? area.width ?? 0,
        height_pct: area.height_pct ?? area.height ?? 0,
  ...
  useEffect(() => {
    ...
    console.debug("[ProductArtworkStudio] active area", {
      id: activeArea.id,
      name: activeArea.name,
      screen_id: activeArea.screen_id,
      activeImage,
      x: areaPct(activeArea, "x"),
      y: areaPct(activeArea, "y"),
      width: areaPct(activeArea, "width"),
      height: areaPct(activeArea, "height"),
      width_mm: activeArea.width_mm,
      height_mm: activeArea.height_mm,
    });
  }, [activeArea, activeImage]);
